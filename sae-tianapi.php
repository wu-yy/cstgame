...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
	$weixin = '测试微信号';  //此处填写你的微信号
	$storage = 'mteam';  //换成你的SAE二级域名，并在“存储与CDN服务-Storage”新建“imgtip”空间
	extract($_POST);
...
*/	
    ...
    $stor->write('imgtip',$savefile,$imgstr);
    ob_end_clean();
    imagedestroy($source);
    imagedestroy($water);
	echo 'http://'. $storage .'-imgtip.stor.sinaapp.com/'.$savefile;
}else{
	echo '-1';
}